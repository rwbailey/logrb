...
import (
	...
	"github.com/rwbailey/logrb/internal/config"
	"github.com/rwbailey/logrb/internal/log"
...
func testConsumePastBoundary(t *testing.T, client api.LogClient, config *Config) {
	...
	assert.Nil(t, consume, "consume response not nil")
...
func testProduceConsumeStream(t *testing.T, client api.LogClient, config *Config) {
	...
	}
	{
		...
		for offset, record := range records {
			...
			res, err := stream.Recv()
			require.NoError(t, err)
			require.Equalf(t, res.Offset, uint64(offset), "got offset: %d, want: %d", res.Offset, offset)
