...
// NewLogService creates a new LogService containing the
// implemented methods of the Log service in s.  Any unimplemented
// methods will result in the gRPC server returning an UNIMPLEMENTED status to the client.
// This includes situations where the method handler is misspelled or has the wrong
// signature.  For this reason, this function should be used with great care and
// is not recommended to be used by most users.
func NewLogService(s interface{}) *LogService {
	ns := &LogService{}
	if h, ok := s.(interface {
		Produce(context.Context, *ProduceRequest) (*ProduceResponse, error)
	}); ok {
		ns.Produce = h.Produce
	}
	if h, ok := s.(interface {
		Consume(context.Context, *ConsumeRequest) (*ConsumeResponse, error)
	}); ok {
		ns.Consume = h.Consume
	}
	if h, ok := s.(interface {
		ConsumeStream(*ConsumeRequest, Log_ConsumeStreamServer) error
	}); ok {
		ns.ConsumeStream = h.ConsumeStream
	}
	if h, ok := s.(interface {
		ProduceStream(Log_ProduceStreamServer) error
	}); ok {
		ns.ProduceStream = h.ProduceStream
	}
	return ns
}

// UnstableLogService is the service API for Log service.
// New methods may be added to this interface if they are added to the service
// definition, which is not a backward-compatible change.  For this reason,
// use of this type is not recommended.
type UnstableLogService interface {
	Produce(context.Context, *ProduceRequest) (*ProduceResponse, error)
	Consume(context.Context, *ConsumeRequest) (*ConsumeResponse, error)
	ConsumeStream(*ConsumeRequest, Log_ConsumeStreamServer) error
	ProduceStream(Log_ProduceStreamServer) error
